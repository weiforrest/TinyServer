...
public:
	 static processpool< M, C >* create(int listenfd, int process_number = 4)
	 {
		  if(!m_instance){
...
/* 统一事件源，使用epoll处理 */
template< typename M, typename C >
void processpool< M, C >::setup_sig_pipe()
{
...
template< typename M, typename C>
void processpool< M, C >::run_child()
{
	 setup_sig_pipe();
					...
					if(((ret < 0) && (errno != EAGAIN)) || ret == 0){ 
						 ...
						 if (connfd < 0){
							  log(LOG_ERR, __FILE__, __LINE__,
								  "accept failure,errno: %s", strerror(errno));
							  continue;
						 }
...
template< typename M, typename C >
void processpool< M, C >::run_parent()
{
	 setup_sig_pipe();
