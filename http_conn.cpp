/* http_conn.cpp    (c) 2014 weiforrest */
#include "http_conn.h"
...
const char *doc_root = "/home/max/Work/Blog";
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
	...
	*m_url++ = '\0';
	...
	if (strcasecmp(method, "GET") == 0) {
		...
		return BAD_REQUEST;	// not support
	}
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
	if (text[0] == '\0') {
		...
		log(LOG_DEBUG, __FILE__, __LINE__, "oop! unknow header %s",
		    text);
	}
	return NO_REQUEST;
}
...
bool http_conn::process_read()
{
	...
	while (true) {
		...
		bytes_read =
			recv(m_sockfd, m_read_buf + m_read_idx,
			     READ_BUFFER_SIZE - m_read_idx, 0);
		if (bytes_read == -1) {
			if (errno == EAGAIN || errno == EWOULDBLOCK) {
				break;
			}
			log(LOG_ERR, __FILE__, __LINE__, "recv error = %s",
			    strerror(errno));
			return false;
